...
set.seed(seed)
# eta_A = c(0, 0.5, -0.2, 0.1, 0.2) #c(A_t_previous, X1, state_t, Z_1, CZ2)
# eta_state = c(0, 0.1, 0.2) #c(-1, 0.1, 0.2)
# beta_interaction = c(0.4, -0.3, 0.3, -0.3, 0, 0)
# gamma_Z = c(0.2, -0.1, -0.1, 0, 0)
...
run_ee <- function(data, p_tilde){
  ...
  ## run GEE
  model2 <- geeglm(Y ~ 1 + state_c + stateZ1_c +
                  Z1*Z2_after,
...
calculate_marginal_effects <- function(solution, cov_mat, solution_mrt_prob, cov_mat_mrt_prob, data_augmented){
  ...
  for (At in At_all) {
    ...
    #   contrast_beta <- c(1, Z_[1], after*Z_[2], after*prod(Z_))
    #   contrast_gamma <- contrast_beta
    #   contrast <- c(contrast_beta*(At-p_tilde), contrast_gamma)
    #   output <- contrasts_geeglm(gee_coef = solution, gee_cov = cov_mat, contrast)
    #   estimate <- output$Estimate
    #   
    #   # responders only have Z1 effects
    ...
    #   truth_nr <- (At-pt_At[2]) * c(1, c(Z_, prod(Z_))) %*% true_coef[1:p_beta_true] + contrast_gamma_true %*% true_coef[1:p_gamma_true+p_beta_true]
    #   truth <- p_responder[(Z_[1]+1)/2+1] * truth_r + (1 - p_responder[(Z_[1]+1)/2+1]) * truth_nr
    #   mc_estimate <- mc_estimate_all[Z1 == Z_[1] & Z2 == Z_[2], V1]
    #   label <- paste0("t >= t^*: E[Y(A = ", At, ", Z = (", Z_[1], ", ", Z_[2], ")]")
    #   dat_marginal_effects <- rbind(dat_marginal_effects, c(label, unlist(output), mc_estimate, truth, estimate - truth))
    # }
  }
  ## END fixed At
  ...
  # before <- 1; after <- 0
  # Z_ <- c(1, 0)
  # # contrast_gamma <- c(1, Z_, prod(Z_))
  # contrast <- c(1, Z_, prod(Z_))
  # output <- contrasts_geeglm(gee_coef = solution_mrt_prob, gee_cov = cov_mat_mrt_prob, contrast)
  # estimate <- output$Estimate
  # # estimate <- contrast %*% solution_combined
  # contrast_true <- c(rep(0, p_beta_true), c(1, Z_, prod(Z_)))
  # truth <- contrast_true %*% true_coef
  # mc_estimate <- data[(Z1 == Z_[1]) & time < T_SMART, mean(Y)]
  # label <- paste0('t < t^*: E[Y(A, Z_1 = ', Z_[1], ")]")
  # coef_labels <- paste_coef(contrast, coef_names)
  # dat_marginal_effects <- rbind(dat_marginal_effects, c(label, coef_labels, unlist(output), mc_estimate, truth, estimate - mc_estimate))
  # 
  # Z_ <- c(-1, 0)
  # # contrast_gamma <- c(1, Z_, prod(Z_))
  # contrast <- c(1, Z_, prod(Z_))
  ...
  # pt_all <- unique(data[(time > T_SMART), .(trt_prob_t, Z1, Z2, responder)])
  # for (Z_index in 1:nrow(Z_all_numeric)) {
  #   # stage 2 estimate averaging over responders and nonresponders
  #   Z_ <- Z_all_numeric[Z_index, ]
  #   contrast <- c(1, Z_, prod(Z_))
  #   output <- contrasts_geeglm(gee_coef = solution_mrt_prob, gee_cov = cov_mat_mrt_prob, contrast)
  #   estimate <- output$Estimate
  ...
  #   # stage 2 estimate for responders
  #   contrast_r1 <- c(rep(0, p_beta_true), (c(1, Z_[1], 0, 0)))
  #   truth_r1 <- contrast_r1 %*% true_coef
  #   contrast_r0 <- c(rep(0, p_beta_true), (c(1, Z_, prod(Z_))))
  #   truth_r0 <- contrast_r0 %*% true_coef
  #   truth <- p_responder[(Z_[1]+1)/2+1] * truth_r1 + 
  #     (1 - p_responder[(Z_[1]+1)/2+1]) * truth_r0
  #   mc_estimate <- data_augmented[(Z1 == Z_[1] & Z2 == Z_[2]) & time >= T_SMART, sum(weight_SMART * Y) / sum(weight_SMART)]
  #   label <- paste0('t >= t^*: E[Y(A, Z = (', Z_[1], ", ", Z_[2], ")]")
  #   coef_labels <- paste_coef(contrast, coef_names)
  #   # cat("\nZ_ =", Z_)
  #   # cat("\nestimate =", estimate)
  #   # cat("\ntruth =", truth)
  ...
  # colnames(dat_marginal_effects) <- c("label", "A", "AZ1", "ACt", "ACtZ2", "ACtZ1", "Intercept", "Z1", "Ct", "CtZ2", "Z1CtZ2", "coef_labels", "estimate", "mc_estimate", "truth", "bias")
