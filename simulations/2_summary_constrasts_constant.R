...
pt_settings <- c("Aconstant") 
pt_setting_labels <- c("$p_t = 0.5$ is constant. ") 
...
for (N in Ns) {
  ...
  for (s in 1:length(pt_settings)) {
    pt_setting <- pt_settings[s]
    # dat_marginal_summary[[pt_setting]] <- dat_marginal[[pt_setting]][, mean(estimate), by = c('method_label', 'contrast_index')] #'label'
