...
for (N in Ns) {
  ...
  for (s in 1:length(pt_settings)) {
    ...
    dat_marginal_summary[[pt_setting]] <- reshape(dat_marginal_summary[[pt_setting]], idvar = c("contrast_index", "label", "mean_truth"), timevar = "method_label", direction = "wide")
  ...
  for (s in 1:length(pt_settings)) {
    ...
    table_1 <- table_1[, .(V1, Stage.Hybrid, Condition.Hybrid, mean_truth, Hybrid, se_Hybrid, cover_Hybrid, WCLS, se_WCLS, cover_WCLS)]
    fileConn <- file(paste0("output/20250401/table_N", N, "_", pt_setting, "_R", responder_setting, "_contrastsA.txt"))
    table_1 %>%
      kbl(caption = " \\footnotesize{Comparison of effects on the proximal outcome between $A_t = 1$ and 0, for a fixed DTR or averaging over DTRs.} ", 
    ...
    table_2 <- table_2[, .(V1, Stage.Hybrid, Condition.Hybrid, mean_truth, Hybrid, se_Hybrid, cover_Hybrid, WR, se_WR, cover_WR, mean_se_ratio.WR, sd_se_ratio.WR)]
    ...
    table_3 <- table_3[, .(V1, Stage.Hybrid, Condition.Hybrid, mean_truth, Hybrid, se_Hybrid, cover_Hybrid)]
